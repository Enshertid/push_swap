...
/*   Updated: 2019/10/08 11:44:45 by ymanilow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_pre_sort_a(t_stack **stack)
{
	...
	{
		ft_reverse_rotate(stack, 1, 1);
		if ((*stack)->num > (*stack)->next->num)
			ft_swap_stack(stack, 1);
	}
	...
	else if ((*stack)->num > (*stack)->next->num && (*stack)->next->num < tmp->num &&
			tmp->num < (*stack)->num)
		ft_rotate_stack(stack, 1,1 );
}
...
void				ft_pre_sort_b(t_stacks *point, t_lst *pre, int num)
{
	if (point->stack_a->num <= pre->array[num] &&
		...
		ft_push_stack(&point->stack_a,&point->stack_b, 1);
	else
